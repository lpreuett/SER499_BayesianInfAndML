...
class KNN_Classifier:
    ...
    def __init__(self, k=1):
        ...
        self.__data_labels = []
        self.__debug = False
        self.__NUM_INPUT_DATA = 2500  # number between 1 and 45211
        if k > 0:
            self.k = k
    ...
    def classify_data(self, input_data):
        ...
        classified_data = numpy.empty([len(input_data[0:self.__NUM_INPUT_DATA])])

        for i in range(0, len(input_data[0:self.__NUM_INPUT_DATA])):
            ...
            print("Step %d of %d" % (i+1, self.__NUM_INPUT_DATA))
    ...
    def standardize_data(self):
        means = self.__data.mean(axis=0)
        stdevs = self.__data.std(axis=0)
